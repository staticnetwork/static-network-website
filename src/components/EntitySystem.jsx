...
import { createRadioTrackShell, syncSocialLiveEvent, toggleSocialFollow, uploadSocialMedia } from '../lib/socialActions'
...
function mediaKindForSignal(type = '', url = '') {
  const signalType = String(type || '').toLowerCase()
  const source = String(url || '').toLowerCase()
  if (signalType.includes('video') || /\.(mp4|webm|mov|m4v)(\?|$)/i.test(source)) return 'video'
  if (signalType.includes('audio') || signalType.includes('music') || /\.(mp3|wav|wave|m4a|aac|ogg|flac|aiff|aif)(\?|$)/i.test(source)) return 'audio'
  return 'image'
}

function renderSignalMediaElement(item) {
  if (!item?.url) return null
  if (item.kind === 'video') return <video className="local-signal-media" src={item.url} controls playsInline preload="metadata" />
  if (item.kind === 'audio') return <div className="local-audio"><SignalMark animated /><audio src={item.url} controls preload="metadata" /></div>
  return <img className="local-signal-media" src={item.url} alt="" />
}
...
function isRadioEligibleSignalType(type = '') {
  const value = String(type || '').toLowerCase()
  return value.includes('music') || value.includes('audio')
}

function mediaAcceptForSignalType(type = '') {
  const value = String(type || '').toLowerCase()
  if (value.includes('music video') || value.includes('video')) return 'video/*'
  ...
  return (
    <article className={`entity-card ${preview ? 'entity-card--preview' : ''}`}>
      ...
      <div className="entity-card__footer">
        ...
        <span>SIGNAL {entity.signalScore || 'PENDING'}</span>
      </div>
    </article>
  )
}
...
export function LocalSignalMedia({ signal, compact = false }) {
  const refs = normalizeMediaList(signal.mediaRefs || (signal.mediaId ? [signal.mediaId] : []))
  const refsKey = refs.join('|')
  const mediaTypeKey = [
    ...normalizeMediaList(signal.fileTypes),
    signal.fileType || signal.postType || signal.type || '',
  ].join('|')
  ...
  useEffect(() => {
    ...
    const mediaTypes = mediaTypeKey ? mediaTypeKey.split('|') : []
    const fallbackType = mediaTypes[mediaTypes.length - 1] || ''
    if (currentRefs.length) {
      Promise.all(currentRefs.map((ref) => getMedia(ref))).then((records) => {
        if (!active) return
        const nextItems = records.filter((record) => record?.blob).map((record, index) => {
          const objectUrl = URL.createObjectURL(record.blob)
          objectUrls.push(objectUrl)
          return {
            url: objectUrl,
            kind: mediaKindForSignal(record.fileType || record.type || mediaTypes[index] || fallbackType),
  ...
  }, [refsKey, mediaTypeKey])

  const cloudItems = normalizeMediaList(signal.mediaUrls || signal.media_urls).map((url, index) => ({
    url,
    kind: mediaKindForSignal(signal.fileTypes?.[index] || signal.fileType || signal.postType || signal.type, url),
    name: signal.fileNames?.[index] || signal.fileName || '',
  }))
  const mediaItems = cloudItems.length ? cloudItems : localItems
  const mediaCount = mediaItems.length
  const safeIndex = Math.min(activeIndex, Math.max(0, mediaCount - 1))
  ...
  if (mediaCount === 1) return renderSignalMediaElement(activeItem)

  if (mediaCount > 1) {
    return (
      <div className="local-signal-carousel" aria-label={`${mediaCount} media items`}>
        {renderSignalMediaElement(activeItem)}
        <button className="local-signal-carousel__control local-signal-carousel__control--prev" type="button" onClick={() => setActiveIndex((current) => (current + mediaCount - 1) % mediaCount)} aria-label="Previous media">‹</button>
        <button className="local-signal-carousel__control local-signal-carousel__control--next" type="button" onClick={() => setActiveIndex((current) => (current + 1) % mediaCount)} aria-label="Next media">›</button>
        <span className="local-signal-carousel__count">{safeIndex + 1}/{mediaCount}</span>
        <span className="local-signal-carousel__dots" aria-hidden="true">
          {mediaItems.map((item, index) => <i className={index === safeIndex ? 'is-active' : ''} key={`${item.url}-${index}`} />)}
        </span>
      </div>
    )
  }
    ...
    <article className="local-signal-card">
      <button type="button" onClick={() => onOpen?.(signal)}>
        ...
        <div className="local-signal-card__body">
          <div><LiveIndicator label="AI POST" /><span>{signal.visibility}</span></div>
          ...
          <div className="local-signal-card__ai">
            <span>AI</span>
            <p>{signal.aiContribution || 'AI-assisted work'}</p>
          </div>
          <footer><span>{signal.type}</span><span>{signal.tags || 'PRIVATE PREVIEW'}</span><em>+{signal.signalReward || signalBoostForType(signal.type)} Signal</em><b>Open Post <ArrowIcon /></b></footer>
        </div>
      </button>
    </article>
  )
...
export function UploadSignalModal({ entity, onClose, initialType = 'Text Signal', firstSignal = false }) {
  ...
  async function publish(event) {
    ...
    saveSignal({
      ...
      signalReward: signalBoostForType(type),
    })
    if (cloudMedia?.publicUrl && isRadioEligibleSignalType(type)) {
      await createRadioTrackShell(user, {
        title: form.get('title') || media?.fileName || `${entity.name} upload`,
        artist: entity.name,
        sourceType: 'creator_upload',
        audioUrl: cloudMedia.publicUrl,
        mediaAssetId: cloudMedia.asset?.id || '',
        genres: String(form.get('tags') || '').split(/[\s,]+/).map((item) => item.replace(/^#/, '').trim()).filter(Boolean).slice(0, 8),
        rightsStatus: 'needs_review',
        visibility: 'private',
        data: {
          mediaMode: String(type).toLowerCase().includes('video') ? 'music_video' : 'audio',
          entityId: entity.id,
          signalType: type,
          publicUrl: cloudMedia.publicUrl,
        },
      }, {
        ownsMaster: true,
        ownsPublishing: true,
        ...
        aiGenerated: true,
        declaration: 'Submitted from a STATIC channel post as AI-made or AI-assisted creator media.',
        status: 'pending_review',
      }).catch(() => null)
    }
    addEntitySignalPoints(entity.id, signalBoostForType(type), `Posted ${type}`)
    setPublishing(false)
    setStatus(cloudMedia?.metadataError ? `Post published. +${signalBoostForType(type)} Signal. Media metadata needs migration: ${cloudMedia.metadataError}` : `Post published. +${signalBoostForType(type)} Signal.`)
    window.setTimeout(onClose, 850)
  }

  const acceptsMedia = ['Video', 'Image', 'Audio', 'Music Drop', 'Music Video'].includes(type)

  return (
    <ModalFrame title={firstSignal ? 'Post First Work' : 'Create Post'} code="POST//COMPOSER" onClose={onClose} wide>
      <form className="signal-composer" onSubmit={publish}>
        <div className="signal-composer__form">
          <div className="signal-composer__rule">
            <span>STATIC NETWORK RULE 01</span>
            <strong>AI-made or AI-assisted work only.</strong>
            <p>The game city is coming. The live network starts here: post the AI work, prove the process, earn Signal.</p>
          </div>
          <label><span>CONTENT TYPE</span><select value={type} onChange={(event) => setType(event.target.value)}>
            {(firstSignal
              ? ['Status', 'Music Drop', 'Music Video', 'Video Clip', 'Live Announcement', 'Game World', 'Marketplace Drop', 'Original Series']
              : ['Text Post', 'Video', 'Music Video', 'Image', 'Audio', 'Link', 'Drop Announcement', 'Live Announcement', 'Game World', 'Original Series']
            ).map((option) => <option key={option}>{option}</option>)}
          </select></label>
          <label><span>TITLE</span><input name="title" required placeholder="Name the post" /></label>
          <label><span>CAPTION / POST TEXT</span><textarea name="caption" rows="5" required placeholder="What are you posting?" /></label>
          ...
          <label><span>AI CONTRIBUTION</span><select name="aiContribution" required>{aiContributionOptions.map((option) => <option key={option}>{option}</option>)}</select></label>
          <label><span>AI TOOL / WORKFLOW</span><input name="aiTools" required placeholder="ChatGPT, Midjourney, Suno, Runway, Blender, custom workflow..." /></label>
          <label><span>PROCESS NOTE</span><textarea name="aiProcess" rows="3" placeholder="What did AI help you create, remix, animate, write, generate, or refine?" /></label>
          <label className="signal-composer__check"><input name="rightsConfirmed" type="checkbox" required /><span>I confirm this uses AI or AI assistance, and I have the right to share it.</span></label>
	          {acceptsMedia && <label className="file-field"><span>UPLOAD {type.toUpperCase()}</span><input type="file" accept={mediaAcceptForSignalType(type)} onChange={chooseFile} required /></label>}
          ...
          <button className="button button--primary button--wide" type="submit" disabled={publishing}>
            {publishing ? 'Posting...' : 'Post'} <ArrowIcon />
          </button>
  ...
  return (
    <div className={`entity-profile ${channelMode ? 'entity-profile--channel' : ''}`} style={{ '--channel-accent': channel?.theme?.accentColor || '#78e8ff' }}>
      ...
      <section className="entity-profile__hero">
        ...
        <span><b>{entity.signalScore}</b>SIGNAL</span>
