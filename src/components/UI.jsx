...
import PremiumMotionLayer from './PremiumMotionLayer'
...
export function StaticBrandMark({ animated = false, className = '' }) {
  return (
    <span className={`static-brand-logo ${animated ? 'static-brand-logo--animated' : ''} ${className}`} aria-hidden="true">
      <img src="/assets/brand/static-mark-official-working.png" alt="" />
    </span>
  )
}
...
export function PageHero({
  ...
  return (
    <section className={`page-hero ${compact ? 'page-hero--compact' : ''} ${image ? 'page-hero--image' : ''} ${className}`} style={style}>
      ...
      <PremiumMotionLayer variant={image ? 'route' : 'signal'} density={compact ? 'quiet' : 'standard'} />
