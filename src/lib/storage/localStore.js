...
const KEYS = {
  ...
  creatorProfile: 'static_creator_profile',
  passkeyRedemptions: 'static_passkey_redemptions',
  notifications: 'static_social_notifications',
}
...
function parseSignalScore(value) {
...
function formatSignalScore(value) {
  if (value >= 1_000_000_000_000) return `${Number((value / 1_000_000_000_000).toFixed(1))}T`
...
export function addEntitySignalPoints(entityId, points = 0, reason = 'Signal activity') {
  const entity = getEntities().find((item) => item.id === entityId)
  if (!entity) return null
  const scoreLocked = entity.isGenesis || /T$/i.test(String(entity.signalScore || ''))
  const nextScore = scoreLocked
    ? entity.signalScore
    : formatSignalScore(parseSignalScore(entity.signalScore) + Number(points || 0))
  const event = {
    id: makeId('score'),
  ...
  return saveEntity({
    ...
    signalScoreEvents: [event, ...(entity.signalScoreEvents || [])].slice(0, 50),
  })
}

export function getCreatorProfile(fallback = {}) {
  const stored = read(KEYS.creatorProfile, null) || {}
  const displayName = stored.displayName || fallback.displayName || fallback.display_name || fallback.email?.split('@')[0] || 'STATIC Creator'
  const handle = stored.handle || fallback.username || fallback.handle || displayName
  const fallbackSignalScore = fallback.signalScore ?? fallback.signal_score
	  return {
	    id: stored.id || fallback.id || 'local-creator',
	    displayName,
	    handle: `@${normalizeHandle(handle)}`,
	    bio: stored.bio || fallback.bio || 'Posting AI-made and AI-assisted work on STATIC.',
	    signalScore: fallbackSignalScore !== undefined && fallbackSignalScore !== null
	      ? formatSignalScore(parseSignalScore(fallbackSignalScore))
	      : stored.signalScore || '0',
	    avatarUrl: stored.avatarUrl || fallback.avatarUrl || fallback.avatar_url || '',
	    avatarRef: stored.avatarRef || fallback.avatarRef || '',
	    bannerUrl: stored.bannerUrl || fallback.bannerUrl || fallback.banner_url || '',
	    bannerRef: stored.bannerRef || fallback.bannerRef || '',
	    signalScoreEvents: stored.signalScoreEvents || [],
...
export function saveCreatorProfile(profile = {}) {
  ...
  const next = {
    ...current,
    ...profile,
    id: current.id && current.id !== 'local-creator' ? current.id : makeId('creator'),
    displayName: profile.displayName || profile.display_name || current.displayName,
    handle: `@${normalizeHandle(profile.handle || profile.username || current.handle)}`,
    updatedAt: new Date().toISOString(),
  }
  write(KEYS.creatorProfile, next)
  return next
}

export function addCreatorSignalPoints(points = 0, reason = 'AI work posted') {
  const creator = getCreatorProfile()
  const score = formatSignalScore(parseSignalScore(creator.signalScore) + Number(points || 0))
  return saveCreatorProfile({
    ...creator,
    signalScore: score,
    signalScoreEvents: [
      { id: makeId('score'), points: Number(points || 0), reason, createdAt: new Date().toISOString() },
      ...(creator.signalScoreEvents || []),
    ].slice(0, 50),
  })
}

const launchPasskeys = [
  {
    code: 'STATIC2000',
    points: 2000,
    label: 'Founder starter passkey',
    description: 'Launch code for early STATIC Social accounts.',
  },
]

function normalizePasskeyCode(value = '') {
  return String(value || '').trim().toUpperCase().replace(/[^A-Z0-9-]/g, '')
}

export function redeemSignalPasskey(inputCode, redeemer = {}) {
  const code = normalizePasskeyCode(inputCode)
  const passkey = launchPasskeys.find((item) => item.code === code)
  if (!code) throw new Error('Enter a passkey first.')
  if (!passkey) throw new Error('That passkey is not active.')

  const redeemerId = redeemer.userId || redeemer.email || getCreatorProfile().id || 'local'
  const redemptionKey = `${redeemerId}:${passkey.code}`
  const redemptions = read(KEYS.passkeyRedemptions, {})
  if (redemptions[redemptionKey]) throw new Error('This account already redeemed that passkey.')

  const nextCreator = addCreatorSignalPoints(passkey.points, `Redeemed passkey ${passkey.code}`)
  write(KEYS.passkeyRedemptions, {
    ...redemptions,
    [redemptionKey]: {
      code: passkey.code,
      points: passkey.points,
      label: passkey.label,
  ...
  addSocialNotification({
    ...
    text: `You gained ${passkey.points.toLocaleString()} Signal from ${passkey.label}.`,
    signalDelta: passkey.points,
    route: '/profile',
  })
...
export function getSocialNotifications() {
  return read(KEYS.notifications, [])
    .filter(Boolean)
    .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
}

export function addSocialNotification(input = {}) {
  const notification = {
    id: input.id || makeId('notification'),
    type: input.type || 'activity',
    title: input.title || 'STATIC activity',
    text: input.text || '',
    actorName: input.actorName || '',
    actorHandle: input.actorHandle || '',
    actorAvatarUrl: input.actorAvatarUrl || '',
    route: input.route || '/notifications',
    signalDelta: Number(input.signalDelta || 0),
    read: Boolean(input.read),
    createdAt: input.createdAt || new Date().toISOString(),
    ...input,
  }
  write(KEYS.notifications, [notification, ...getSocialNotifications()].slice(0, 120))
  return notification
}

export function markSocialNotificationsRead() {
  return write(KEYS.notifications, getSocialNotifications().map((item) => ({ ...item, read: true })))
}
